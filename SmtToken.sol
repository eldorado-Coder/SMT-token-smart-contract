...
contract SmtToken is Ownable, IBEP20 {
    ...
    address constant BURN = 0x0C4fb0b49C53Db9E0dCCF815Fdd4D0d187c67d48;
    address public marketingFeeReceiver;
    address public smartFeeReceiver;
    uint256 totalFee = 10;
    uint256 marketingFee = 1;
    uint256 burnFee = 2;
    uint256 buySmartFee = 9;
    uint256 sellSmartFee = 7;
    uint256 feeDenominator = 100;

    constructor(address marketingFeeAddress, address smartFeeAddress) {
        marketingFeeReceiver = marketingFeeAddress;
        smartFeeReceiver = smartFeeAddress;
    ...
    function _takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(sender.isContract() && keccak256(bytes(IPancakePair(sender).symbol())) == keccak256(bytes("Cake-LP"))) {
            if(IPancakePair(sender).token0() == address(this) || IPancakePair(sender).token1() == address(this)) {
                ...
                uint256 smartFeeAmount = amount * buySmartFee / feeDenominator;
                _balances[smartFeeReceiver] += smartFeeAmount;
                emit Transfer(sender, smartFeeReceiver, smartFeeAmount);
        ...
        if(receiver.isContract() && keccak256(bytes(IPancakePair(receiver).symbol())) == keccak256(bytes("Cake-LP"))) {
            if(IPancakePair(receiver).token0() == address(this) || IPancakePair(receiver).token1() == address(this)) {
                ...
                uint256 smartFeeAmount = amount * sellSmartFee / feeDenominator;
                _balances[smartFeeReceiver] += smartFeeAmount;
                emit Transfer(sender, smartFeeReceiver, smartFeeAmount);
     ...
     */
    function setSmartFeeReceiver(address account) external onlyOwner {
        smartFeeReceiver = account;
    }
